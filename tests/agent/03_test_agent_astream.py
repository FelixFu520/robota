...
async def test_model_stream_direct():
    """直接测试模型的流式输出"""
    print('test_model_stream_direct (直接测试模型流式):')
    print('=' * 60)
    ...
    async for chunk in model.astream("介绍一下你自己"):
    ...
    print('\n' + '=' * 60)
    if chunk_count > 0:
        total_time = last_chunk_time - first_chunk_time if last_chunk_time else 0
        print(f"总共收到 {chunk_count} 个chunk")
        if chunk_count > 1:
            avg_interval = total_time / (chunk_count - 1) if chunk_count > 1 else 0
            print(f"平均chunk间隔: {avg_interval:.3f} 秒")
            if avg_interval < 0.1:
                print("✓ 模型流式输出正常")
            else:
                print("⚠ 模型流式输出间隔较长")
...
def test_astream():
    """测试agent的流式输出 - 使用同步stream方法，检查工具调用"""
    print('test_astream (测试agent流式输出和工具调用):')
    print('=' * 60)
    
    user_message = "你好，介绍你自己；你都可以调用那些工具；然后再计算 1 + 2 等于多少"
    
    # 测试1: 使用stream_mode="messages"查看文本流式输出
    print("\n【测试1】stream_mode='messages' - 查看文本流式输出:")
    print("-" * 60)
    chunk_count = 0
    first_chunk_time = None
    last_chunk_time = None
    ...
    print("\n【测试2】stream_mode='updates' - 查看工具调用过程:")
    print("-" * 60)
    ...
    if tool_call_count > 0:
        ...
        print("  ✗ Agent没有调用工具（可能工具调用被合并到文本中）")
    print()


if __name__ == "__main__":
    # asyncio.run(test_model_stream_direct())
    test_astream()