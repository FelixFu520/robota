...
class AudioDevice:
    ...
    def _print_device_info(self):
        ...
        print("="*60)
        print(f"\n输入设备:")
        print(f"  设备名称: {self.input_device_info['name']}")
        print(f"  设备索引: {self.input_device_index}")
        print(f"  最大输入通道: {self.input_device_info['maxInputChannels']}")
        print(f"  默认采样率: {self.input_device_info['defaultSampleRate']}")
        print(f"\n输出设备:")
        print(f"  设备名称: {self.output_device_info['name']}")
        print(f"  设备索引: {self.output_device_index}")
        print(f"  最大输出通道: {self.output_device_info['maxOutputChannels']}")
        print(f"  默认采样率: {self.output_device_info['defaultSampleRate']}")
        print(f"\n音频配置:")
        print(f"  当前采样率: {self.sample_rate}")
    ...
    def generate_test_audio(self, duration=3, frequency=440):
        ...
        Returns:
            音频数据列表
        """
        print(f"\n生成测试音频: {duration}秒, 频率={frequency}Hz")
    ...
    def play_audio(self, audio_chunks):
        ...
        """
        print(f"开始播放音频... (共{len(audio_chunks)}块)")
        for chunk in audio_chunks:
            self.playback_queue.put(chunk)
    
    def record_audio(self, duration=5, save_path="output.wav"):
        """
        录制音频
        ...
        print(f"\n开始录音: {duration}秒")
        if self.aec is not None:
            print("提示: 录音时会应用高级回声消除算法（NLMS + 谱减法）")
        else:
            print("提示: 回声消除已禁用")
