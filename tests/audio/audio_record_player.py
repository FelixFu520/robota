...
"""
音频录音和播放测试程序
支持同时进行音频播放和录音，并可选择启用回声消除(AEC)功能
用于测试 AudioDevice 类的功能
"""
import argparse
import time
import wave
...
class AudioRecorderPlayer:
    ...
    def _playback_loop(self):
        ...
        # 计算每个chunk的字节数
        # chunk_size * channels * 2 bytes (int16格式)
        bytes_per_chunk = self.device.chunk_size * self.device.channels * 2
    ...
    def _save_wav_file(self, filename, audio_data):
        ...
        Args:
            filename: 输出文件名路径
            audio_data: 音频数据(bytes格式，16位PCM)
        ...
        try:
            with wave.open(filename, 'wb') as wf:
                # 设置WAV文件参数
                wf.setnchannels(self.device.channels)  # 声道数
                wf.setsampwidth(2)  # 16-bit = 2 bytes
                wf.setframerate(self.device.sample_rate)  # 采样率
                # 写入音频数据
    ...
    def start(self):
        ...
        # 启动播放线程（守护线程，主程序退出时自动结束）
        self.playback_thread = threading.Thread(target=self._playback_loop, daemon=True)
        self.playback_thread.start()
        
        # 启动录音线程（守护线程，主程序退出时自动结束）
    ...
    def run(self):
        """
        运行系统（阻塞直到用户中断）
        启动系统后持续运行，定期显示状态信息，直到用户按Ctrl+C中断
        """
        self.start()
...
def main():
    """
    主函数
    解析命令行参数，初始化音频设备和录音播放管理器，运行测试程序
    """
    ...
    parser.add_argument('--playback-file', type=str, 
                        ...
                        help='循环播放的音频文件路径')
    parser.add_argument('--output-dir', type=str, default='./assets/recordings',
